const WRITE_METHODS = ['insertOne', 'insertMany', 'updateOne', 'updateMany', 'replaceOne', 'deleteOne', 'deleteMany', 'drop', 'createIndex', 'createIndexes', 'dropIndex', 'dropIndexes']
  ...
  return new Proxy(db, {
    get (target, prop) {
      if (DB_WRITE_METHODS.includes(prop)) {
        return (...args) => {
          log?.('info', 'migrations', `[DRY RUN] db.${prop}(${args.map(a => JSON.stringify(a)).join(', ')})`)
        }
      }
      if (prop === 'collection') {
  ...
  return new Proxy(collection, {
    get (target, prop) {
      if (WRITE_METHODS.includes(prop)) {
        return (...args) => {
          log?.('info', 'migrations', `[DRY RUN] ${name}.${prop}(${args.map(a => JSON.stringify(a)).join(', ')})`)
        }
      }
