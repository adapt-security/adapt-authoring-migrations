...
class MigrationsModule extends AbstractModule {
  ...
  async executeConfMigration (migration, { dryRun }) {
    const prefix = dryRun ? '[DRY RUN] ' : ''
    const files = await glob('conf/*.config.js', { cwd: this.app.rootDir, absolute: true })
    for (const filePath of files) {
      ...
      if (dryRun) {
        this.log('info', `${prefix}would write ${fileName}`)
        continue
      }
      await fs.writeFile(filePath, `export default ${JSON.stringify(config, null, 2)}\n`, 'utf8')
      this.log('info', `updated ${fileName}`)
    }
    if (!dryRun) {
      await this.recordCompleted(migration)
    }
    ...
    for (const [moduleName, files] of Object.entries(fileMap)) {
      for (const filePath of files) {
        ...
        if (type === 'conf') {
          ...
          migrations.push({ module: moduleName, version, type, description: configMigration.description, configMigration })
        } else {
