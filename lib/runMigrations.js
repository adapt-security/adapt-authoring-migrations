...
export async function runMigrations (options) {
  ...
  try {
    ...
    for (const m of pending) {
      ...
      try {
        await executeMigration(m, { readOnlyConfig, dryRun, client, db, useTransactions, rootDir, log })
      } catch (error) {
...
async function executeMigration (migration, ctx) {
  const { readOnlyConfig, dryRun, client, db, useTransactions, rootDir, log } = ctx
  if (migration.type === 'conf') {
    return executeConfMigration(migration, { readOnlyConfig, dryRun, db, rootDir, log })
  }
  if (useTransactions) {
...
async function executeConfMigration (migration, { readOnlyConfig, dryRun, db, rootDir, log }) {
  const prefix = dryRun ? '[DRY RUN] ' : ''
  ...
  for (const filePath of files) {
    ...
    if (readOnlyConfig) {
      log('warn', 'migrations', `[READ-ONLY CONFIG] ${fileName} requires manual config changes for ${migration.module}@${migration.version}`)
      logConfigDiff(config, configClone, log, 'warn')
      continue
    }
    if (dryRun) {
      log('info', 'migrations', `${prefix}would write ${fileName}`)
      logConfigDiff(config, configClone, log, 'info')
      continue
    }
    await fs.writeFile(filePath, `export default ${JSON.stringify(configClone, null, 2)}\n`, 'utf8')
    log('info', 'migrations', `updated ${fileName}`)
  }
  if (!dryRun && !readOnlyConfig) {
    await recordCompleted(db, migration)
  }
}
    ...
    if (!before[mod]) {
      ...
      for (const key of allKeys) {
        if (!(key in before[mod])) {
          log(level, 'migrations', `  + ${mod}.${key}: ${JSON.stringify(after[mod][key])}`)
        } else if (!(key in after[mod])) {
          log(level, 'migrations', `  - ${mod}.${key}`)
        } else if (JSON.stringify(before[mod][key]) !== JSON.stringify(after[mod][key])) {
          log(level, 'migrations', `  ~ ${mod}.${key}: ${JSON.stringify(before[mod][key])} -> ${JSON.stringify(after[mod][key])}`)
        }
      }
    }
  }
